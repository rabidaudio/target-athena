...
| partition_keys | List(String) | No | List of the partition keys. If not specified, the Athena table(s) are created without partitions. There is no current way to specify different partition keys for different tables.  If you have need of different partition keys for different tables the suggestion solution is to process the records separately. |
